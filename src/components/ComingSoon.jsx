...
// import { SlSocialInstagram } from "react-icons/ai";
...
function ComingSoon() {
	...
	async function sendEmail(e) {
		...
		if (!isValid) {
			...
			try {
				setIsLoading(true);
				await Api.post('/waiting', JSON.stringify({ email }), {
					headers: {
		...
		<div className="bg-[#BBC4D9] h-screen w-screen flex flex-col ">
			...
			<div className="flex flex-col mt-32 md:mt-52 items-center p-2 ">
				...
				<p className="font-mono text-center mb-4">
					{' '}
					Kejepay is an escrow service to ensure trust for every of your online
					transaction
				</p>
				...
				<form className="flex xs:flex-col md:flex-row">
					<div className="flex flex-col sm:flex-row">
						<input
							type="email"
							className="bg-white text-black border-1 sm:rounded-l-lg mt-2 px-4 py-2"
						...
						<button
							type="button"
							disabled={isLoading}
							className="bg-black text-white  sm:rounded-r-lg px-4 py-2 float-right mt-2"
							onClick={sendEmail}>
							{' '}
							NOTIFY ME
						</button>
